package fanpeihua.justforfun.base.fbase.utils.base;
...
public class ToastUtils {
    ...
    private static int yOffset = (int) (64 * MyApplication.getsInstance().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes int layoutId) {
        LayoutInflater inflate = (LayoutInflater) MyApplication.getsInstance().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    private static void show(@StringRes int resId, int duration) {
        show(MyApplication.getsInstance().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(MyApplication.getsInstance().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        cancel();
        if (customView != null) {
            sToast = new Toast(MyApplication.getsInstance());
            sToast.setView(customView);
            sToast.setDuration(duration);
        } else {
            sToast = Toast.makeText(MyApplication.getsInstance(), text, duration);
        }
